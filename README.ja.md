...
Outlook向けのMCPサーバーは少なくありません。カレンダーや連絡先まで含む全部入りのものが複数あり、それらは送信もできます。そして**送信を実装しないという同じ判断に、独立して到達しているサーバーが他にもあります。** 正しい判断であり、隠さず書いておきます。

そのうえで、**他に見当たらなかった点**はこれです。

| | |
|---|---|
| **フォルダ階層の外科手術** | `move_folder` は**サブツリーごと**動かします。数千通の居場所が**API呼び出し1回**で変わり、メッセージIDは失効せず、そのフォルダを指す振り分けルールも生き続けます。フォルダを作れるサーバーはありますが、階層を組み替えられるものは見つかりませんでした |
| **振り分けルールを一級のツールとして扱う** | 読み取り・作成・削除。Outlookの Web UI で作った既存ルールも正しく解釈します(UIが書く `fromAddresses` 形式は、API で作成するときの形式とは別物です) |
| **書き込みの全停止スイッチ** | `OUTLOOK_READONLY=true` で全ての書き込みツールを一括無効化。「見せるが触らせない」運用ができます |

以下は、この領域の良いサーバーと**共通する**性質です。どちらが先かに関わらず、明記しておきます。

- 送信ツールが存在せず `Mail.Send` も要求しない
- 完全削除が無く、削除はゴミ箱まで
- 一括処理は既定が下見

### 他の選択肢

用途が合わなければ、こちらが向いているかもしれません。

- **[littlebearapps/outlook-mcp](https://github.com/littlebearapps/outlook-mcp)** — カレンダー・連絡先まで含む全部入り。送信もできますが、下見・レート制限・宛先allowlistで守る方式です。Outlook全体を1つで賄いたいならこちら
- **[ajs117/outlook-mcp](https://github.com/ajs117/outlook-mcp)** — 個人アカウント特化で、同じく送信しません。**RFC 8058 のワンクリック配信停止を含むニュースレター整理機能**があり、これは本サーバーにはありません。`bulk_by_query` がメッセージIDを一切会話に流さない設計も見事です
...
| `mark_read_by_search` | 破壊 | 条件に一致するメールを一括既読化(最大25,000件・**元に戻せない**) |
