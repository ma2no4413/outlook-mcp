...
| | |
|---|---|
| 実施日 | 2026-08-14（初回実施 2026-08-10 / 別マシンへ移設後の再実行） |
| 結果 | **64項目 / 64パス、0失敗** |
| 内訳 | 単体テスト 35項目 + スモークテスト 10項目 + **結合テスト（実アカウント）読み取り9項目・書き込み10項目** |
...
## 1. 単体テスト（[test_outlook.py](../test_outlook.py) / 35項目）
...
| S-02 | ツールの公開 | 9個、名前が定義どおり | PASS |
| S-03 | 入力スキーマ | 各ツールの `required` がシグネチャどおり | PASS |
| S-04 | 操作種別の申告 | `read_only_hint` / `destructive_hint` が正しい（`move_to_trash` のみ destructive） | PASS |
...
- レート制限（429）に到達したときの挙動
- 実際のHTMLメールに対する平文化の読みやすさ（4節のとおり `html_to_text()` が発火せず）
- 25件を超える一括操作の実挙動（上限拒否は S-09 で確認済み。上限内での大量操作は未実施）
...
| ファイル | 内容 |
|---|---|
| [evidence/pytest.txt](evidence/pytest.txt) | `pytest -v` の全出力（35項目の個別結果） |
