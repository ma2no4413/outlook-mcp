...
| | |
|---|---|
| 実施日 | 2026-08-14（初回実施 2026-08-10 / 別マシンへ移設後の再実行 / 同日 v0.2.0 でツール追加） |
| 結果 | **109項目 / 109パス、0失敗** |
| 内訳 | 単体テスト 80項目 + スモークテスト 10項目 + **結合テスト（実アカウント）読み取り9項目・書き込み10項目** |
| 環境 | Windows 11 Home 10.0.22631 / Python 3.13.1 / mcp 2.0.0 / msal 1.37.0 / httpx 0.28.1 |

> **v0.2.0 でツールが 9 → 19 に増えました（2026-08-14）。**
> 追加分（フォルダ改名/移動/削除・一括移動・一括既読化・振分ルール3種・下書き2種）の
> 単体テストは 45項目を新規に書き、6節にまとめています。
> スモークテストも公開ツール19個に更新しました。
> 追加分は実アカウント（数万通規模）に対する**実運用でも通していますが**、
...
> v0.2.0 で追加した45項目は **6節** にまとめています。合計80項目です。
...
| 規模 | 3桁のフォルダ数・最大3階層・日本語名を含む構成 |
| エビデンス | [evidence/integration.txt](evidence/integration.txt) |

> 稼働中のメールボックスなので件数は実行のたびに動きます（本節の実行中にも受信トレイが2件増えました）。エビデンスには**件名・差出人・フォルダ名の実データを記録していません**。加えて、ある時点の件数・フォルダ数といった**メールボックスの規模を特定できる数値も伏せています**。このリポジトリは公開前提のため、返り値の「形」と判定に必要な構造だけを残しています。

| ID | 検証内容 | 結果 | 備考 |
|---|---|---|---|
| I-01 | `login.py` の device code flow | PASS | 下記「踏んだ不具合」参照。**README の手順に誤りがあり、修正した** |
| I-02 | `check_config` の疎通 | PASS | 受信トレイの総数・未読数を取得 |
| I-03 | 日本語フォルダ名・階層の解決 | PASS | 3桁のフォルダを列挙。`親/12.英数字混じり/日本語の名前` のような**3階層＋日本語＋記号＋アポストロフィ**を含むパスを正しく解決 |
...
## 6. v0.2.0 追加ツールの検証（2026-08-14）

ツールを 9 → 19 に増やしました。単体テストを45項目追加し、**実アカウント（数万通規模）で実運用も通しています**。ただし4〜5節のように手順化した結合テストとしては記録していないため、確認できた範囲と残っている範囲を分けて書きます。
...
| 下書き | `POST /me/messages` を叩き `/sendMail` を叩かないこと、宛先・Cc の組み立て、宛先/件名が空の呼び出しの拒否、返信は `createReply` / 全員返信は `createReplyAll`、読み取り専用時の拒否 |
| **送信スコープ** | **`SCOPES` に `Mail.Send` が含まれないこと**。このプロジェクトの中核となる保証で、壊れたら送信できてしまう |

> **S-05 の判定基準を変えました。** 以前はツール名に `send` / `reply` / `forward` / `draft` が
> 含まれないことを見ていましたが、下書き機能はそれらの語を正当に使います。
> ツール名の検査は補助でしかなく、本当の保証は**要求スコープに `Mail.Send` が無いこと**です。
> 権限が無ければ、コードが壊れても・エージェントが乗っ取られても送信は起きません。
> 判定をそちらへ移し、単体テストにも同じ検査を入れました。

### 実運用で確認できたこと

| 項目 | 実績 |
|---|---|
| フォルダの改名・移動 | 3桁のフォルダを9トップレベルへ再編。**フォルダIDは改名・移動をまたいで維持される**ことを確認（既存の振分ルール31件が移動後も正しい移動先を指し続けた） |
