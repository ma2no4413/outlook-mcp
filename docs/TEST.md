...
| 結果 | **107項目 / 107パス、0失敗** |
| 内訳 | 単体テスト 78項目 + スモークテスト 10項目 + **結合テスト（実アカウント）読み取り9項目・書き込み10項目** |
...
> v0.2.0 で追加した43項目は **6節** にまとめています。合計78項目です。
...
ツールを 9 → 19 に増やしました。単体テストを43項目追加し、**実アカウント（数万通規模）で実運用も通しています**。ただし4〜5節のように手順化した結合テストとしては記録していないため、確認できた範囲と残っている範囲を分けて書きます。
